...
H_d = 0.4  # demagnetization field along z axis 0.5
...
def module_evolution(time_step, m_x, m_y, m_z, current_density):
    """
    ...
    :return: the matrix of all of m_x, the last state of m vector
    """
    ...
    for i1 in range(1, time_step):
        ...
        c = gamma * H_d * m_y * m_z + u0 * gamma * h_dl * (m_y ** 2 + m_z ** 2) - \
            alpha * m_z * (-gamma * (H_x + H_k * m_x + H_d * m_x) * m_z - u0 * gamma * h_dl * m_x * m_y)
        d = alpha * m_y - (alpha ** 2) * m_x * m_z
        e = 1 + (alpha ** 2) * (m_x ** 2)
        f = gamma * (H_x + H_k * m_x) * m_y - u0 * gamma * h_dl * m_x * m_z + alpha * m_x * (
                -gamma * (H_x + H_k * m_x + H_d * m_x) * m_z - u0 * gamma * h_dl * m_x * m_y)
        d_mx1 = (b * f + c * e) / ((1 + (alpha ** 2) * (m_z ** 2)) * e + b * d)
        d_mz1 = ((1 + (alpha ** 2) * (m_z ** 2)) * f - c * d) / ((1 + (alpha ** 2) * (m_z ** 2)) * e + b * d)
        d_my1 = -gamma * (H_x + H_k * m_x + H_d * m_x) * m_z - u0 * gamma * h_dl * m_x * m_y + alpha * m_z * d_mx1 \
                - alpha * m_x * d_mz1
        ...
        C = -gamma * (H_x + H_k * m_x + H_d * m_x) * m_z
        D = -u0 * gamma * h_dl * m_x * m_y
        E = gamma * (H_x + H_k * m_x) * m_y
    ...
    return matrix_mx, mx_1, my_1, mz_1


input_signals = list(np.random.randint(0, 2, 40))
input_voltage = []
...
# input_signals = [-0.7, -0.7, -0.7, 0.45, 0.45, 0.45, -0.7]
...
for i in input_voltage:
    if i > 0:
        mx_list, m_x0, m_y0, m_z0 = module_evolution(50000, m_x=m_x0, m_y=m_y0, m_z=m_z0, current_density=i)
